...
namespace { // private
	...
	public:
		...
		{
			CSftpDirectory dir(sandbox_pidl().get(), Provider());
...
void do_filedescriptor_test(
	T fixture, const com_ptr<IDataObject>& data_object,
	const vector<wpath>& files)
{
	FORMATETC fetc = {
	...
	{
		BOOST_CHECK_EQUAL(fgd[i].path(), files[i].filename());
		BOOST_CHECK_EQUAL(
			fgd[i].file_size(), file_size(fixture->resolve(files[i])));
		BOOST_CHECK_EQUAL(
			fgd[i].last_write_time(),
			from_time_t(last_write_time(fixture->resolve(files[i]))));
	}
}
...
void do_filecontents_test(
	T fixture, const com_ptr<IDataObject>& data_object,
	const vector<wpath>& files, int index)
{
	FORMATETC fetc = {
	...
	BOOST_CHECK(file_stream_equivalence(fixture->resolve(files[index]), stream));
}
...
BOOST_AUTO_TEST_CASE( file_descriptor )
{
	...
	do_filedescriptor_test(this, data_object, files);

	for (size_t i = 0; i < files.size(); ++i)
		do_filecontents_test(this, data_object, files, i);
}
...
/**
 * Ask for the FILEDESCRIPTOR format where one file is a link.
 * Ensure the link returns the same data as the real file.
 */
BOOST_AUTO_TEST_CASE( file_descriptor_symlink )
{
	vector<wpath> files = make_test_files();
	create_link(files[0], L"some-symlink");
	files.push_back(files[0].parent_path() / L"some-symlink");
 ...
 * Ensure the link returns the same data as the real file.
 */
BOOST_AUTO_TEST_CASE( file_descriptor_broken_symlink )
{
	vector<wpath> files = make_test_files();
	create_link(files[0], L"some-symlink");
	files.push_back(files[0].parent_path() / L"some-symlink");
