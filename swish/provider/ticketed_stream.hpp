...
{
public:
    ticketed_stream(com_ptr<IStream> stream,
                    swish::connection::session_reservation&& ticket)
        : m_ticket(ticket), m_inner(stream)
    {
    }

    virtual HRESULT STDMETHODCALLTYPE Read(void* buffer, ULONG buffer_size,
                                           ULONG* read_count_out)
    {
        return m_inner->Read(buffer, buffer_size, read_count_out);
    }

    virtual HRESULT STDMETHODCALLTYPE Write(const void* data, ULONG data_size,
                                            ULONG* written_count_out)
    {
        return m_inner->Write(data, data_size, written_count_out);
    }

    virtual HRESULT STDMETHODCALLTYPE Seek(LARGE_INTEGER offset, DWORD origin,
    ...
    virtual HRESULT STDMETHODCALLTYPE CopyTo(IStream* destination,
                                             ULARGE_INTEGER amount,
                                             ULARGE_INTEGER* bytes_read_out,
                                             ULARGE_INTEGER* bytes_written_out)
    {
        return m_inner->CopyTo(destination, amount, bytes_read_out,
                               bytes_written_out);
    }
    ...
    virtual HRESULT STDMETHODCALLTYPE LockRegion(ULARGE_INTEGER offset,
                                                 ULARGE_INTEGER extent,
                                                 DWORD lock_type)
    {
        return m_inner->LockRegion(offset, extent, lock_type);
    }

    virtual HRESULT STDMETHODCALLTYPE UnlockRegion(ULARGE_INTEGER offset,
                                                   ULARGE_INTEGER extent,
                                                   DWORD lock_type)
    {
    ...
    virtual HRESULT STDMETHODCALLTYPE Stat(STATSTG* attributes_out,
                                           DWORD stat_flag)
    {
