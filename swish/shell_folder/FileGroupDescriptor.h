...
#include "RemotePidl.h"  // PIDL handling
...
public:
	 ...
	 */
	ATL::CString GetPath() throw()
	{
		ATL::CString strPath = cFileName;
		WinToUnix(strPath);
		return strPath;
	}

	static inline void UnixToWin(ATL::CString& strPath) throw()
	{
		strPath.Replace(L"/", L"\\");
	}

	static inline void WinToUnix(ATL::CString& strPath) throw()
	{
		strPath.Replace(L"\\", L"/");
	}
};
