...
namespace detail {
    ...
    inline swish::connection::connection_spec connection_from_host_item(
        const host_itemid_view& host_item)
    {
        return swish::connection::connection_spec(
            host_item.host(), host_item.user(), host_item.port());
    }
...
public:
    overlay_icon(const winapi::shell::pidl::cpidl_t& item)
        :
    m_connection(
        detail::connection_from_host_item(host_itemid_view(item)))
    {}
